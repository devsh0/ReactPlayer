    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      audioList.push(
        <div
          ...
          onClick={() => props.onAudioSelected(media)}
        >
          <div className={"title"}>{getFormattedMediaName(media.name)}</div>
          <div className={"metadata"}>
            <span className={"duration"}>
              {getFormattedMediaTime(media.duration)}
            </span>
            <button
              title={"Remove"}
              className={"remove"}
      ...
      <div className={"header-container"}>
        ...
        <div className={"button-container"}>
          ...
          <button
            ...
            onClick={props.onPlaylistCleared}
          >
            Clear
          </button>
        </div>
      </div>
      <div className={"playlist-container"}>{getAudioRows()}</div>
    </div>
  );
}
